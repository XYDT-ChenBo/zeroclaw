...
use crate::dt_nodes_registry::handle_http_response;
use crate::channels::{
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    if config.gateway.a2a.enabled {
        a2a::init(&config, &display_addr, tools_registry.as_ref())?;
    }
    ...
    println!("  GET  {pfx}/          — WebSocket for node connections");
    println!("  POST {pfx}/response  — HTTP agent chat (non-streaming)");
...
async fn run_gateway_chat_with_tools(
    ...
    if config.gateway.node_control.enabled {
        extra_tools.push(Box::new(crate::tools::NodesTool::new(&config.workspace_dir)));
    }
